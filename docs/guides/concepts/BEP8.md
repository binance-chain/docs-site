...
* TokenType: **1** for `tiny token` and **2** for `Mini-BEP2 token`. Total supply range of tiny token is [1-10K] and mini-token is [1, 100M]. Mini-token will charge more than tiny-token for issue.
...
```bash
./bnbcli token mint --amount 100000000000000000 --symbol $mini-token-symbol --from alice --chain-id Binance-Chain-Tigris   --node  https://dataseed5.defibit.io:443  --trust-node
```
