...
- [Node RPC](#node-rpc)
  * [1. Connecting](#1-connecting)
    + [1.1 Use your own local node](#11-use-your-own-local-node)
    + [1.2 Use an existing node on the network](#12-use-an-existing-node-on-the-network)
...
```bash
$ bnbcli dex show -l NNB-0AB_BNB --chain-id Binance-Chain-Nile --node data-seed-pre-2-s1.binance.org:80
```
